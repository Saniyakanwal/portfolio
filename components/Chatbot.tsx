...
export default function ChatBot() {
  ...
  const [messages, setMessages] = useState([
    { role: "bot", text: "Hello! I am Saniya Kanwal's assistant!" }
  ]);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] font-sans">
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-[300px] sm:w-[350px] h-[500px] bg-[#030014]/95 backdrop-blur-xl border border-white/10 rounded-[2rem] flex flex-col shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden animate-in fade-in zoom-in duration-300">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-3 rounded-2xl text-xs leading-relaxed break-words whitespace-pre-wrap ${
                  m.role === 'user' 
            ...
            {isTyping && (
              <div className="text-[10px] text-gray-400 italic ml-2 animate-pulse">AI is thinking...</div>
            )}
          </div>
